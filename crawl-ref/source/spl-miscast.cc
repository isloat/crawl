...
void MiscastEffect::do_miscast()
{
    ...
    if (special_source == HELL_SHARED_MISCAST)
    {
        _hellshare();
        return;
    }

    if (special_source == HELL_SPECIFIC_MISCAST)
    {
        _hellspecific();
        return;
    }
...
void MiscastEffect::_charms(int severity)
{
    ...
    case 3:         // potentially lethal
        ...
        {
        case 0:
            target->paralyse(act_source, 2 + random2(6));
            break;
        case 1:
            target->confuse(act_source, 5 + random2(3));
            break;
...
void MiscastEffect::_hellshare()
{
    ...
    case 4:
        ...
        case 0:
            if (target->holiness() & MH_UNDEAD)
            {
                all_msg      = "Something just walked over your grave."
                               "No, really!";
                do_msg();
            }
            else
        ...
        case 2:
            if (_create_monster(summon_any_demon(RANDOM_DEMON_GREATER), 0, true))
                ...
                all_msg = "You sense a faint hostile presence.";
            break;
        case 3:
            all_msg = "Venomous gases billow around you!";
            _big_cloud(CLOUD_POISON, 16 + random2(6), 16 + random2(6));
            break;
        }
        break;
    case 5:
    ...
    case 7:
        ...
        {
        case 0:
            lose_stat(STAT_RANDOM, 3 + random2avg(3, 2));
            break;
        case 1:
            contaminate_player(2000 + random2avg(12000, 2), spell != SPELL_NO_SPELL);
            break;
        case 2:
            _malign_gateway();
            break;
        case 3:
            {
                bool success = false;
                for (int i = 2 + random2(3); i > 0; --i)
                    success |= _create_monster(coinflip() ? MONS_ABOMINATION_LARGE :
                                                            MONS_VERY_UGLY_THING, 5, true);

                if (success)
                    all_msg = "A chorus of gurgles and moans calls out from beyond!";
                else
                    all_msg = "Gurgles and moans pass briefly through your mind.";
                do_msg();
            }
        }
        break;
    ...
    case 9:
        ...
        case 2:
            if (you.magic_points > 0)
            {
                ...
                all_msg = "Magic surges around you!";
            break;
        case 3:
            target->blink();
            break;
...
void MiscastEffect::_hellspecific()
{
    switch (random2(5))
    {
    case 0:
    case 1:    // attuned explosions
        {
            beam.damage = dice_def(4, 16);
            beam.ex_size = 1;
            beam.glyph   = dchar_glyph(DCHAR_FIRED_BURST);
            ...
            {
                mpr("There is a sudden explosion of frost!");
                beam.name = "icy explosion";
                beam.flavour = BEAM_COLD;
                beam.colour  = WHITE;
            }
            else if (player_in_branch(BRANCH_DIS))
            {
                mpr("There is a sudden explosion of flying shrapnel!");
                beam.name = "explosion";
                beam.flavour = BEAM_FRAG;
                beam.colour  = LIGHTCYAN;
            }
            ...
            {
                mpr("There is a sudden explosion of ghostly flame!");
                beam.name = "ghostly flames";
    ...
    case 4:
        ...
        if (player_in_branch(BRANCH_GEHENNA))
        {
            ...
            {
                mpr("You are blasted with searing flames! The searing flames burn away your fire resistance!");
                if (_ouch(18 + random2avg(24, 2), BEAM_FIRE) && target->alive())
                    target->expose_to_element(BEAM_FIRE, 10);
                if (!target->alive())
                    break;
                you.increase_duration(DUR_FIRE_VULN, 15 + random2(11), 50);
            ...
            {
                all_msg = "You are covered in liquid flames!";
                napalm_player(random2avg(7,3) + 1, cause);
            }
        }
        else if (player_in_branch(BRANCH_COCYTUS))
        {
            if (x_chance_in_y(2, 3))
            {
                mpr("You are encased in ice!");
                if (_ouch(18 + random2avg(24, 2), BEAM_ICE) && target->alive())
                    target->expose_to_element(BEAM_COLD, 10);
                if (!target->alive())
                    break;
                you.increase_duration(DUR_FROZEN, 6 + random2(11), 50);
            }
            ...
            {
                all_msg = "Heat is siphoned rapidly from your body!";
                you.increase_duration(DUR_NO_POTIONS, 10 + random2(11), 50);
            }
        }
        else if (player_in_branch(BRANCH_DIS))
        {
            if (x_chance_in_y(2, 3))
            {
                mpr("Hooked metal barbs suddenly pierce through your skin!");
                _ouch(18 + random2avg(24, 2), BEAM_FRAG);
                if (!target->alive())
                    break;
            ...
            {
                target->corrode_equipment("malicious rust");
            }
        }
        else if (player_in_branch(BRANCH_TARTARUS))
        {
            if (x_chance_in_y(2, 3))
            {
                mpr("You are enveloped in negative energy!");
                _ouch(18 + random2avg(24, 2), BEAM_NEG);
            }
            else
            {
                mpr("An unnatural silence engulfs you.");
